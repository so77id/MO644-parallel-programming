...
void run(md5bench_t* args) {
    for(int i = 0; i < args->iterations; i++) {
        #pragma omp single nowait
        {
            ...
            #pragma omp task shared(args) firstprivate(buffers_to_process, next, in, out)
            {
...
int main(int argc, char** argv) {
    ...
    TIME(b_start);
    #pragma omp parallel num_threads(nt) shared(args)
    {
        run(&args);
    }
...
Each sample counts as 0.01 seconds.
 ...
 78.73      5.78     5.78     5116     1.13     1.13  body
 21.29      7.34     1.56                             initialize
  0.00      7.34     0.00     2558     0.00     1.13  MD5_Final
  0.00      7.34     0.00     2558     0.00     0.00  MD5_Init
  0.00      7.34     0.00     2558     0.00     1.13  MD5_Update


  Index by function name

   [3] MD5_Final               [4] MD5_Update              [5] initialize
   [6] MD5_Init                [1] body (md5.c)
